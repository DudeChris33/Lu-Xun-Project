...
PROJECT_TITLE = "Snake — Lu Xun Edition"
PROJECT_SUBTITLE = "Christopher Cyr - UC Irvine - Hist 170E"
	...
	def _draw(self) -> None:
		...
		for i, (gx, gy) in enumerate(self.state.snake):
			...
			if i == 0:
				...
				glyph = self.theme.snake_body_glyph
			canvas.create_text(
				cx, cy, text=glyph,
				font=(EMOJI_FONT_NAME, CELL_SIZE - 4),
				fill=self.theme.palette.text,
			)
